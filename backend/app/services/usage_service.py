#user\backend\app\services\usage_service.py
...
def check_and_consume(username: str, required_mb: int, usage_type: str, db: Session):
    ...
    total_used = (usage.download_used_mb or 0) + (usage.api_used_mb or 0)
    ...
    if remaining < required_mb:
        raise HTTPException(
            status_code=403,
            detail="No data balance left. Please buy addon."
        )

    if usage_type == "download":
        usage.download_used_mb += required_mb
    else:
        usage.api_used_mb += required_mb

    usage.total_limit_mb = total_limit
    usage.remaining_mb = total_limit - (
        usage.download_used_mb + usage.api_used_mb
    )
